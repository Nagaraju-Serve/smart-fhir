...
// const filteredData = data.filter(entry => entry.resource.status === "active")
//     .map(entry => {
//         const reasonCode = entry?.resource?.reasonCode[0];
//         const codeObject = reasonCode.coding.find(code => code.system.includes('icd-10'));
//         return {
//             code: codeObject ? codeObject.code : '',
//             text: entry?.resource?.code?.text,
//             display: codeObject ? codeObject.display : ''
//         };
//     });
...
//     if (item.reasonCode && item.reasonCode.length > 0) {
//         const icd10Code = item.reasonCode.find(codeObj => codeObj.coding.some(c => c.system.includes('icd-10')));
//         if (icd10Code) {
//             code = icd10Code.coding[0].code;
//             display = icd10Code.coding[0].display;
//         }
//     }
